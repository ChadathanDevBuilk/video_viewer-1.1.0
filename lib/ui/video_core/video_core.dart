...
class _VideoViewerCoreState extends State<VideoViewerCore> {
  ...
  void _forwardDragStart(DragStartDetails details) {
    final video = _query.video(context);
    if (!video.isShowingSettingsMenu && _pointers == 1 && video.isFullScreen)
      setState(() {
  ...
  void _forwardDragEnd() {
    final video = _query.video(context);
    if (!video.isShowingSettingsMenu && _showForwardStatus && video.isFullScreen) {
  ...
  void _volumeDragEnd() {
    final video = _query.video(context);
    if (!video.isShowingSettingsMenu && _showVolumeStatus && video.isFullScreen) {
      setState(() {
